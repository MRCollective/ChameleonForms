ChameleonForms = window.hasOwnProperty("ChameleonForms") ? ChameleonForms : {};

(function (ChameleonForms) {

    ChameleonForms.isValidDate = function (value, format) {
    ...
    ChameleonForms.isValidTime = function (value, format) {
    ...
    ChameleonForms.RegisterAspNetValidationDateProvider = function(v) {
        v.addProvider('date', (value, element, params) => {
            ...
            var format = element.getAttribute("data-val-format");
            if (!format)
            ...
            return ChameleonForms.isValidDate(valueSplitBySpaces[0], formatSplitBySpaces[0])
                && (formatSplitBySpaces.length === 1 || ChameleonForms.isValidTime(valueSplitBySpaces[1], formatSplitBySpaces[1]));
        });
    };

})(ChameleonForms);

if (window.hasOwnProperty('aspnetValidation') && !ChameleonForms.skipAspNetValidationRegistration) {

    var v = new window.aspnetValidation.ValidationService();

    ChameleonForms.RegisterAspNetValidationDateProvider(v);
